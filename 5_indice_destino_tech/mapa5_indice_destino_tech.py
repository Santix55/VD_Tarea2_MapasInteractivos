...
MOBILITY_FILE = BASE_DIR / "2_movilidad_y_transporte" / "salidas" / "mapa2_movilidad_transportes_datos.csv"
MAP2_SCRIPT = BASE_DIR / "2_movilidad_y_transporte" / "mapa2_movilidad_y_transporte.py"
...
def load_climate_by_province() -> pd.DataFrame:
    if not CLIMATE_FILE.exists():
        raise FileNotFoundError(
            "No se encontro la cache climatica. Ejecuta primero el mapa 4 o coloca "
            f"el archivo en {CLIMATE_FILE}."
        )
    if not PRECIPITATION_FILE.exists():
        raise FileNotFoundError(
            "No se encontro la cache de precipitacion. Ejecuta primero el mapa 4 o coloca "
...
def add_scores(data: gpd.GeoDataFrame) -> gpd.GeoDataFrame:
    ...
    scored["mobility_score"] = scored["mobility_score"].clip(0, 100)
...
def add_labels(data: gpd.GeoDataFrame, bins: list[float], current_year: int) -> gpd.GeoDataFrame:
    ...
    labelled["mobility_label"] = labelled["mobility_score"].map(
        lambda value: "sin dato" if pd.isna(value) else f"{float(value):.1f} / 100"
    )
    labelled["nearest_strategic_label"] = labelled["nearest_strategic_km"].map(
        lambda value: "sin dato" if pd.isna(value) else f"{float(value):.0f} km"
    )
...
def save_static_map(map_data: gpd.GeoDataFrame, current_year: int, bins: list[float]) -> None:
    ...
    map_ax.set_title(
        "Equilibrio provincial entre vivienda, movilidad, conectividad y confort",
        fontsize=16.2,
        fontweight="bold",
        pad=12,
    )
    ...
    fig.suptitle(
        f"Mapa 5. Indice final de destino residencial tech ({current_year})",
        fontsize=20,
        fontweight="bold",
        x=0.44,
        y=0.985,
    ...
    fig.text(
        ...
        "Renfe Data, CNIG/IGN red ferroviaria, AENA/ENAIRE aeropuertos, "
    ...
    fig.savefig(OUTPUT_DIR / "mapa5_indice_destino_tech.png", bbox_inches="tight")
    fig.savefig(OUTPUT_DIR / "mapa5_indice_destino_tech.pdf", bbox_inches="tight")
    plt.close(fig)
          ...
          function popupHtml(properties, weights) {
            ...
            return `
              <div style="font-family: Arial, sans-serif; font-size: 12px; line-height: 1.35; min-width: 260px;">
                ...
                Movilidad: <strong>${formatter.format(asNumber(properties.mobility_score))} / 100</strong><br>
                Nodo estrategico cercano: <strong>${formatter.format(asNumber(properties.nearest_strategic_km))} km</strong><br>
          ...
          function tooltipHtml(properties) {
            return `
              <div>
                ...
                Movilidad: ${formatter.format(asNumber(properties.mobility_score))} / 100<br>
                Nodo estrategico cercano: ${formatter.format(asNumber(properties.nearest_strategic_km))} km<br>
...
def save_interactive_map(
    ...
    web_map.save(OUTPUT_DIR / "mapa5_indice_destino_tech_interactivo.html")
...
def save_tables(map_data: gpd.GeoDataFrame, current_year: int) -> None:
    ...
    table.to_csv(OUTPUT_DIR / "mapa5_indice_destino_tech_datos.csv", index=False)
...
def main() -> None:
    ...
    print(f"Mapa 5 generado con alquiler, movilidad, conectividad y clima ({current_year}).")
