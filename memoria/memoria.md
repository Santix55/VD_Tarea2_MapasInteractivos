...
La decisión de dónde vivir después de un máster en inteligencia artificial tiene una dimensión espacial clara. Un perfil técnico puede buscar empleo presencial o híbrido, pero también puede teletrabajar si la infraestructura digital del territorio lo permite. Al mismo tiempo, el alquiler condiciona la viabilidad real de cualquier destino: una provincia con mucha actividad tecnológica puede no ser atractiva si el coste de entrada es demasiado alto o si el mercado se está tensionando con rapidez.
...
> Qué provincias ofrecen mejor equilibrio entre coste de vivienda, movilidad intermodal, conectividad para teletrabajo y confort climático para un perfil universitario de IA.
...
La memoria no busca producir cinco mapas independientes, sino una narrativa de decisión. Por eso cada mapa responde a una pregunta distinta:
...
3. Dónde la conectividad permite teletrabajo avanzado.
4. Qué provincias tienen condiciones climáticas más confortables.
5. Qué destinos salen mejor al integrar todos los criterios.
...
Para representar mapas web se mantuvo `EPSG:4326`. Cuando se calcularon distancias o puntos representativos se usaron reproyecciones adecuadas para evitar errores geométricos en las capas publicadas en Folium.
...
- Intervalos definidos por usuario cuando existe una lectura operativa, como la cobertura de 1 Gbps.
...
![Mapa 2. Movilidad y transporte](../2_movilidad_y_transporte/salidas/mapa2_movilidad_transportes.png)
...
Salida interactiva: `../3_conectividad_teletrabajo/salidas/mapa3_conectividad_teletrabajo_interactivo.html`.

## Mapa 4. Confort climático estacional

![Mapa 4. Confort climático estacional](../4_confort_climatico/salidas/mapa4_confort_climatico_estacional.png)

El cuarto mapa incorpora una dimensión de calidad de vida. Se calcula la temperatura media estacional entre 1995 y 2024 y se deriva un índice de confort climático. Esta variable tiene menos peso que la vivienda o la conectividad, pero ayuda a distinguir destinos que serían similares en términos económicos.
...
Salida interactiva: `../5_indice_destino_tech/salidas/mapa5_indice_destino_tech_interactivo.html`.
...
Por eso, el índice final debe entenderse como una herramienta de apoyo a la decisión, no como una respuesta única. Su utilidad está en ordenar variables comparables y hacer visibles los compromisos entre vivienda, movilidad, conectividad y confort climático. La mejor provincia no es necesariamente la que obtiene la puntuación más alta para todos los casos, sino la que encaja mejor con el proyecto vital y profesional de cada persona.

# Apéndice técnico: ejecución y regeneración

El proyecto puede ejecutarse desde la raíz del repositorio. Si se usa el entorno Conda preparado para la práctica, se puede activar primero con `conda activate VD` o indicar el intérprete mediante `PYTHON_BIN=/home/s/miniconda3/envs/VD/bin/python`.

La forma más directa es ejecutar cada script de mapa por separado:
...
En Linux o macOS también se puede usar la carpeta `run/`, que contiene lanzadores Bash. Para abrir todos los HTML ya generados:

```bash
./run/abrir_todos_los_mapas.sh
```

Para regenerar todos los mapas sin abrir el navegador:

```bash
./run/regenerar_todos_los_mapas.sh
```

También se puede regenerar un mapa concreto, por ejemplo:

```bash
./run/mapa3_conectividad_teletrabajo.sh --regen --no-abrir
```

Para Windows, o para evitar depender de Bash, existe una versión equivalente en `run_python/`. Permite abrir todos los mapas con:
...
O ejecutar un lanzador individual:

```bash
python run_python/mapa2_movilidad_y_transporte.py --regen --no-abrir
```

Todos los scripts dejan sus salidas dentro de la carpeta `salidas/` de cada mapa, normalmente en formato PNG, PDF, HTML interactivo y CSV.
