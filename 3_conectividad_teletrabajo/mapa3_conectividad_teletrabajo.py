...
def save_static_map(map_data: gpd.GeoDataFrame) -> None:
    ...
    for band in SATELLITE_BANDS:
        band_lon_lat = [(lon, lat) for lat, lon in band]
        map_ax.add_patch(
            Polygon(
                band_lon_lat,
                closed=True,
    ...
    map_ax.set_xlim(*STATIC_MAIN_BOUNDS["xlim"])
    map_ax.set_ylim(*STATIC_MAIN_BOUNDS["ylim"])
    map_ax.set_title(
        f"Peninsula y Baleares: cobertura {tech['short']} y area satelital",
        fontsize=15.5,
        fontweight="bold",
        pad=12,
    )
    ...
    legend_handles = [
        ...
        mpatches.Patch(facecolor="#e31a1c", edgecolor="#c40000", alpha=0.28, label="Conexion satelital general"),
    ]
    map_ax.legend(
    ...
    map_ax.text(
        0.025,
        0.97,
        "Conexion terrestre + satelite\n"
        f"WiFi/fijo: {tech['short']}  |  umbral {STATIC_THRESHOLD}%\n"
        "Capa satelital: general y conceptual",
    ...
    fig.suptitle(
        "Mapa 3. Conectividad multitecnologia para teletrabajo",
        fontsize=19,
        fontweight="bold",
        x=0.43,
        y=0.985,
    ...
    fig.savefig(OUTPUT_DIR / "mapa3_conectividad_teletrabajo.png", bbox_inches="tight")
    fig.savefig(OUTPUT_DIR / "mapa3_conectividad_teletrabajo.pdf", bbox_inches="tight")
    plt.close(fig)
...
def save_interactive_map(map_data: gpd.GeoDataFrame) -> None:
    ...
    web_map.save(OUTPUT_DIR / "mapa3_conectividad_teletrabajo_interactivo.html")
...
def save_tables(map_data: gpd.GeoDataFrame, stats: dict[str, int]) -> None:
    ...
    table["year"] = 2024
    table.to_csv(OUTPUT_DIR / "mapa3_conectividad_teletrabajo_datos.csv", index=False)

    pd.DataFrame([stats]).to_csv(
        OUTPUT_DIR / "mapa3_conectividad_teletrabajo_validacion_union.csv",
        index=False,
    )
...
def main() -> None:
    ...
    print("Mapa 3 generado como conectividad municipal multitecnologia.")
    print(
        "Union municipal: "
