...
def save_static_map(map_data: gpd.GeoDataFrame) -> None:
    ...
    fig.suptitle(
        f"Mapa 4. Confort climatico por epoca del ano ({START_YEAR}-{END_YEAR})",
        fontsize=18,
        fontweight="bold",
        y=0.98,
    )
    ...
    fig.savefig(OUTPUT_DIR / "mapa4_confort_climatico_estacional.png", bbox_inches="tight")
    fig.savefig(OUTPUT_DIR / "mapa4_confort_climatico_estacional.pdf", bbox_inches="tight")
    plt.close(fig)
...
def add_dispersion_legend(web_map: folium.Map, map_data: gpd.GeoDataFrame) -> None:
    ...
    html = f"""
    <div style="
      position: fixed; bottom: 170px; left: 28px; z-index: 9999;
      background: rgba(255,255,255,0.94); padding: 10px 12px;
      border: 1px solid #999; border-radius: 4px;
      font-family: Arial, sans-serif; font-size: 12px; line-height: 1.35;
      box-shadow: 0 1px 5px rgba(0,0,0,0.25);">
...
def save_interactive_map(map_data: gpd.GeoDataFrame) -> None:
    ...
    web_map.save(OUTPUT_DIR / "mapa4_confort_climatico_estacional_interactivo.html")
...
def save_tables(map_data: gpd.GeoDataFrame) -> None:
    ...
    table.to_csv(OUTPUT_DIR / "mapa4_confort_climatico_estacional_datos.csv", index=False)
...
def main() -> None:
    ...
    print(
        f"Mapa 4 generado con temperaturas mensuales, estaciones y precipitacion {START_YEAR}-{END_YEAR}."
    )
